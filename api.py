...
def update_soul_txt(public_url):
    try:
        with open("bgmi1", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in bgmi1")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("bgmi1", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated bgmi1 on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update bgmi1 on VPS: {str(e)}")
