...
def configure_ngrok():
    ngrok_token = "2pydGDSjyBaSgArEadrP9GzIiO3_7v5cSk645zyC7YU2CRDWN"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("jony5.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in jony5.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jony5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated jony5.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update jony5.txt on VPS: {str(e)}")
