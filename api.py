...
def update_soul_txt(public_url):
    try:
        with open("bgmi5.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in bgmi5.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("bgmi5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated bgmi5.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update bgmi5.txt on VPS: {str(e)}")
